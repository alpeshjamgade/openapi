...
func (r *RedisCache) GetAccessToken(ctx context.Context, key string) (string, error) {
	normalizedKey := normalize(key)
	result, err := r.Client.Get(ctx, normalizedKey).Result()
	if err != nil {
		return "", err
...
func (r *RedisCache) SaveAccessToken(ctx context.Context, key string, value interface{}, duration time.Duration) error {
